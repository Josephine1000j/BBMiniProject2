...
input_folder = 'C:\\Users\\josep\\PycharmProjects\\BBMiniProject\\King Domino dataset\\Cropped and perspective corrected boards'
output_folder = 'C:\\Users\\josep\\PycharmProjects\\BBMiniProject\\Processed Tiles'  # Mappe til gemte billeder
os.makedirs(output_folder, exist_ok=True)  # Opret mappen, hvis den ikke eksisterer

# Gennemgå alle billeder i input-mappen
for filename in os.listdir(input_folder):
    if filename.endswith('.jpg'):  # Tjek om filen er et billede
        ...
        print(f"Billedets størrelse: {width}x{height} pixels for {filename}")
        ...
        if height == 500 and width == 500:
            ...
            # Gem den kombinerede billede med alle tiles
            output_image_path = os.path.join(output_folder, f'combined_{filename}')
            cv2.imwrite(output_image_path, combined_image)
            print(f"Gemte kombineret billede til: {output_image_path}")
        else:
            print(
                f"Billedet {filename} har ikke den forventede størrelse. Det er {height}x{width} pixels, ikke 500x500.")
